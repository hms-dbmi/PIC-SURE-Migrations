...
INSERT
    INTO access_rule (
        ...
        @allDictRequests, 'ALLOW_DICTIONARY_DUMP', 'Permit requests to dictionary endpoints',
        '$.[\'Target Service\']', 11, '^/proxy/dictionary-dump/.*$', 0x00, 0x00, NULL, 0x00, 0x00
    );
-- Add that access rule to the DATA_ADMIN privilege
SET @uuidPriv = (SELECT uuid FROM privilege WHERE name = 'AGGREGATE');
INSERT
